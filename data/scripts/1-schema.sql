...
-- Default tenant seed data → see 5-insert.sql
...
-- Subscription plan seed data → see 5-insert.sql
...
-- v_tenant_subscription_status view → see 4-views.sql

--
-- Stored procedure for subscription validation
-- sp_validate_tenant_subscription → see 3-procedures.sql
...
-- Stand-in tables and legacy mysqldump ALGORITHM=UNDEFINED view exports removed.
-- All views (vw_campaign_analytics, v_daily_readmission_summary, v_readmission_analytics)
-- are defined in 4-views.sql using CREATE OR REPLACE VIEW.
...
-- Inventory views (v_low_stock_items, v_expiring_items, v_inventory_valuation) → see 4-views.sql
